...
class WebSocket extends Swoole\Protocol\WebSocket
{
    ...
    {
//        $this->log("timer trigger! \n");
        //每秒检测，如有更新则推送。

        if(count($this->connections) <= 0) return ;

        $file_path_prefix = __DIR__.DIRECTORY_SEPARATOR."data".DIRECTORY_SEPARATOR;
        //todo 可改为循环遍历目录；就无需手动配置，加异常文件的容错处理
        $file_arr = array("btcchina.json", "huobi.json", "okcoin.json", "chbtc.json", "bitstamp.json", "btce.json");

        foreach($file_arr as $key => $value) {
            swoole_async_readfile($file_path_prefix.$value, function($filename, $content){

                $tmp = json_decode($content, true);
                if(!isset($tmp["time"])) return ;
                if(!isset($this->timestamp[$tmp['key']])) $this->timestamp[$tmp['key']] = 0;
                //todo 有些自己生成的time的就没办法比对了；
                if($tmp["time"] == $this->timestamp[$tmp['key']]){
                    return ;
                }
                $this->timestamp[$tmp['key']] = $tmp['time'];


                //推送到每个fd
                foreach ($this->connections as $clid => $info)
                {
                    $this->send($clid, $content);
                }
            });
